...
PSQL="psql --username=chuck --dbname=worldcup --no-align --tuples-only -c"
...
echo "$($PSQL "SELECT winners.name AS team_name FROM games FULL JOIN teams AS winners ON games.winner_id = winners.team_id WHERE round='Eighth-Final' AND year=2014 UNION SELECT opponents.name AS team_name FROM games FULL JOIN teams AS opponents ON games.opponent_id = opponents.team_id WHERE round='Eighth-Final' AND year=2014 ORDER BY team_name")"
